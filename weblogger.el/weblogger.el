...
like."
  (let ((postid      (cdr (assoc-ignore-case "postid" response)))
	...
	(wp_slug         (cdr (assoc-ignore-case "wp_slug" response)))
...
(defun weblogger-struct-to-request (entry)
  ...
  (delq nil
	(list
	 ...
	 (cons "wp_slug"              (cdr (assoc "wp_slug"         entry)))
...
(defun message-goto-Permlink ()
  "Move point to the permlink header."
  (interactive)
  (message-position-on-field "Permlink" "Subject"))

(defun message-goto-Tags ()
  "Move point to the tags header."
  (interactive)
  (message-position-on-field "Tags" "Subject"))

(defun message-goto-Categories ()
